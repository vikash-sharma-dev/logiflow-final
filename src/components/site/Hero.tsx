    ...
    <div className="relative mx-auto grid min-h-[88vh] max-w-7xl items-center px-6 pb-20 pt-16 lg:grid-cols-[1.05fr_1fr] lg:px-12 lg:pt-20">
      <div className="max-w-2xl animate-fade-up">
        ...
        <p className="mt-7 max-w-xl text-base leading-relaxed text-foreground/75 sm:text-lg">
          Logiflow Pvt. Ltd. connects businesses with multiple courier and cargo partners
          to ensure reliable, cost-effective, and timely deliveries — all from a single point of contact.
        </p>
        ...
        <div className="mt-9 flex flex-wrap items-center gap-4">
          ...
          <Button size="lg" variant="outline" className="h-14 rounded-xl border-navy/20 bg-white px-8 text-base font-semibold text-navy hover:bg-navy hover:text-white">
            <MessageCircle /> Chat on WhatsApp
          </Button>
        </div>

        <p className="mt-6 text-sm font-medium text-foreground/60">
          Reliable pan-India logistics for B2B, e-commerce, and retail businesses.
        </p>

        <div className="mt-10 flex flex-wrap items-center gap-x-8 gap-y-3 text-sm font-medium text-foreground/70">
          <div className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-teal" /> Multi-courier routing</div>
          <div className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-teal" /> 27,000+ pin codes</div>
          <div className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-teal" /> Since 1959 legacy</div>
        </div>
      </div>
    </div>
  </section>
