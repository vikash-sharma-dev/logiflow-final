...
export const FinalCta = () => (
  <section id="contact" className="relative isolate overflow-hidden bg-cta-gradient py-28 text-white">
    ...
    <div className="relative mx-auto grid max-w-6xl items-center gap-12 px-6 lg:grid-cols-[1.2fr_1fr] lg:px-12">
      ...
      <form className="relative rounded-3xl border border-white/10 bg-white p-8 text-navy shadow-lift sm:p-9" onSubmit={(e) => e.preventDefault()}>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-bold uppercase tracking-[0.2em] text-teal">Avg. response · under 2 hrs</span>
        </div>
        <h3 className="mt-3 font-display text-[1.7rem] font-extrabold leading-tight tracking-tight text-navy">Get a tailored quote</h3>
        <p className="mt-1.5 text-sm text-muted-foreground">Two fields. Real human callback — no chatbots.</p>
        <div className="mt-7 space-y-4">
          <div>
            <label className="text-[11px] font-bold uppercase tracking-[0.18em] text-navy/70" htmlFor="name">Name</label>
            <input id="name" type="text" required maxLength={100} className="mt-2 h-12 w-full rounded-xl border border-border bg-white px-4 text-[15px] text-navy placeholder:text-muted-foreground/80 transition focus:border-teal focus:outline-none focus:ring-4 focus:ring-teal/15" placeholder="Your name" />
          </div>
          <div>
            <label className="text-[11px] font-bold uppercase tracking-[0.18em] text-navy/70" htmlFor="phone">Phone</label>
            <input id="phone" type="tel" required maxLength={15} className="mt-2 h-12 w-full rounded-xl border border-border bg-white px-4 text-[15px] text-navy placeholder:text-muted-foreground/80 transition focus:border-teal focus:outline-none focus:ring-4 focus:ring-teal/15" placeholder="+91 90000 00000" />
          </div>
          <Button type="submit" size="lg" className="group relative mt-3 h-14 w-full overflow-hidden rounded-xl bg-accent-gradient text-base font-semibold text-white shadow-glow transition hover:scale-[1.01] hover:opacity-95">
            <span className="relative z-10 flex items-center gap-2">Request a Call Back <Phone className="h-4 w-4 transition group-hover:translate-x-0.5" /></span>
          </Button>
